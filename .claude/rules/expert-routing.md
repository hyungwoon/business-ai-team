...
5. **학습 지식 읽기 (2단계 — 각각 별도 Read 호출 필수)**
   - **5a.** `knowledge/[도메인].md` 읽기 — 도메인별 보정/노하우/주의사항 반영
   - **5b.** `knowledge/preferences.md` 읽기 — 도메인 공통 선호도 반영 (톤, 스타일, 구조 등)
   - 5a와 5b는 **독립된 Read 호출**로 각각 실행. 하나만 읽고 넘어가는 것은 절차 위반
   - 보정 사항은 SKILL.md보다 우선
6. **전문가 관점 적용** — 에이전트 시스템 프롬프트 + 스킬 베스트 프랙티스 + 학습 지식 보정 반영하여 응답
7. **담당 표시 (필수)** — 응답 맨 앞에 `> **담당**: [에이전트] | 참조 스킬: [스킬]`
...
- **절대 금지**: `knowledge/preferences.md` 읽지 않고 비즈니스 응답 생성 — 도메인 지식(5a)과 별도로 반드시 읽기(5b)
